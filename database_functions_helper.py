...
def register_loyalty_program(passenger_username):
    """Register a passenger for the loyalty program."""
    connection = sqlite3.connect('project_data.db')
    cursor = connection.cursor()
    ...
    try:
        ...
        cursor.execute("SELECT loyalty_id FROM LoyaltyProgram WHERE passenger = ?", (passenger_username,))
        if cursor.fetchone():
            connection.close()
            return False, "Passenger already enrolled in loyalty program"
        
        # Generate loyalty ID
        loyalty_id = generate_id('LOY')
        
        # Insert loyalty program record
        cursor.execute("""
        INSERT INTO LoyaltyProgram (loyalty_id, passenger, points, membership_level)
        VALUES (?, ?, 0, 'Basic')
        """, (loyalty_id, passenger_username))
        
        connection.commit()
        connection.close()
        return True, "Passenger registered for loyalty program successfully"
